...
class RuleBasedBaseline(Model):
    ...
    def evaluate(self, test_data = True):
        ...
        if test_data:
            input_set = zip(self.datacreator_instance.x_test,
                            self.datacreator_instance.y_test)
        else:
            input_set = zip(self.datacreator_instance.x_train,
                            self.datacreator_instance.y_train)
        ...
        for utterance, dialog_act in input_set:
            for rule in self.rule_data["rules"]:
                if any(keyword in utterance for keyword in rule["keywords"]) and dialog_act in rule["intent"]:
                    ...
                    break
            counter += 1
        print("Rule-based baseline accuracy: ", str(round(correct_counter / counter * 100, 2)) + "%")
